...
class ContigMap():
    ...
    def __init__(self, parsed_pdb, contigs=None, contig_atoms=None, inpaint_seq=None, inpaint_str=None, length=None,
                 ref_idx=None, hal_idx=None, idx_rf=None, inpaint_seq_tensor=None, inpaint_str_tensor=None, topo=False):
        # sanity checks
        ...
        if ref_idx is None:
            ...
            self.atomize_indices2atomname = {self.ref.index(res_num): atom_names for res_num, atom_names in
                                             self.atomize_resnum2atomnames.items()}
    ...
    def expand_sampled_mask(self):
        ...
        for con in self.sampled_mask:
            if (all([i[0].isalpha() for i in con.split(",")[:-1]]) and con.split(",")[-1] == '0') or self.topo is True:
                # receptor chain
                subcons = con.split(",")[:-1]
                assert all([i[0] == subcons[0][0] for i in
                            subcons]), "If specifying fragmented receptor in a single block of the contig string, they MUST derive from the same chain"
                assert all(int(subcons[i].split("-")[0][1:]) < int(subcons[i + 1].split("-")[0][1:]) for i in range(
                    len(subcons) - 1)), "If specifying multiple fragments from the same chain, pdb indices must be in ascending order!"
                for idx, subcon in enumerate(subcons):
                    ref_to_add = [(subcon[0], i) for i in
                                  np.arange(int(subcon.split("-")[0][1:]), int(subcon.split("-")[1]) + 1)]
                    receptor.extend(ref_to_add)
                    receptor_hal.extend(
                        [(self.receptor_chain, i) for i in np.arange(receptor_idx, receptor_idx + len(ref_to_add))])
                    receptor_idx += len(ref_to_add)
                    if self.contig_atoms is not None:
                        atomize_resnum2atomnames.update({(k[0], int(k[1:])): v for k, v in self.contig_atoms.items() if
                                                         (k[0], int(k[1:])) in receptor})
                ...
                for subcon in con.split(","):
                    if subcon[
                        0].isalpha():  # this is a part of the motif because the first element of the contig is the chain letter
                        ref_to_add = [(subcon[0], i) for i in
                                      np.arange(int(subcon.split("-")[0][1:]), int(subcon.split("-")[1]) + 1)]
                        inpaint.extend(ref_to_add)
                        inpaint_hal.extend([(chain_order[inpaint_chain_idx], i) for i in
                                            np.arange(inpaint_idx, inpaint_idx + len(ref_to_add))])
                        inpaint_idx += len(ref_to_add)
                        if self.contig_atoms is not None:
                            atomize_resnum2atomnames.update(
                                {(k[0], int(k[1:])): v for k, v in self.contig_atoms.items() if
                                 (k[0], int(k[1:])) in inpaint})
                    else:
                        inpaint.extend([('_', '_')] * int(subcon.split("-")[0]))
                        inpaint_hal.extend([(chain_order[inpaint_chain_idx], i) for i in
                                            np.arange(inpaint_idx, inpaint_idx + int(subcon.split("-")[0]))])
    ...
    def get_mappings(self):
        ...
        mappings['con_hal_pdb_idx'] = [self.inpaint_hal[i] for i in range(len(self.inpaint_hal)) if
                                       self.inpaint[i] != ("_", "_")]
        ...
        if self.inpaint != self.ref:
            ...
            mappings['complex_con_hal_pdb_idx'] = [self.hal[i] for i in range(len(self.hal)) if
                                                   self.ref[i] != ("_", "_")]
            mappings['receptor_con_ref_pdb_idx'] = [i for i in self.receptor if i != ("_", "_")]
            mappings['receptor_con_hal_pdb_idx'] = [self.receptor_hal[i] for i in range(len(self.receptor_hal)) if
                                                    self.receptor[i] != ("_", "_")]
